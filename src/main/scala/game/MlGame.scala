...
case class MlGame(player: User, game: Running, hiLoGameEvaluator: Evaluator[HiLoGame]):
  ...
  private def generateAiDecision(game: Running): MLCore =
    val first = if (game.discardPile.length >= 1) game.discardPile(0) else 0
    val second = if (game.discardPile.length >= 2) game.discardPile(1) else 0
    val third = if (game.discardPile.length >= 3) game.discardPile(2) else 0
    val fourth = if (game.discardPile.length >= 4) game.discardPile(3) else 0
    val fifth = if (game.discardPile.length >= 5) game.discardPile(4) else 0

    val gameState = HiLoGame(first, second, third, fourth, fifth)
    val gameStateWithPrediction = hiLoGameEvaluator.evaluate(gameState)
    if (gameStateWithPrediction.bestChoice == 1)
      game.betMore
    else
      game.betLess
